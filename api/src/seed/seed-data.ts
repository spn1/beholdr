...
const fetchDataFromApi = async (path: string) => {
  ...
  const creatures = await Promise.all(indexes.map(async index => {
...
export const insertSeedDataFromApi = async (context: KeystoneContext<TypeInfo>) => {
  const creatures = await fetchDataFromApi('/monsters?name=dragon');
  const reducedCreatures = selectCreatureData(creatures);

  await Promise.all(reducedCreatures.map(async (creature: Creature) => {
